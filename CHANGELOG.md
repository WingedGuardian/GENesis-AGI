...
- **The worktree-removal guard no longer refuses a command for *mentioning* a
  ...
  the executable, its subcommand and its real arguments. Measured over 48,363
  real session commands, the swap frees 4 mention-only blocks and closes 6
  genuine removals that were previously allowed: a different and strictly better
  set, not a smaller one.

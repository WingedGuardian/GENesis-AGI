...
| `codex-at-head` | `BLOCK — … — substitute available: <reviewer> reviewed this head` | Codex has not covered the head, but Devin or CodeRabbit has. With the owner's approval, merge with `# substitute-review`: the gate asks the owner itself, keeps the base check and the head binding, and refuses in a dispatched session. Asking the owner is not optional — the gate enforces it. |

...
3. For software capability questions, search for existing implementations before proposing a build. Use `recon_github_search` for repositories, code, and issues; use `recon_github_read` to inspect repository metadata, trees, and source. Apply `/evaluate` for the final adopt/adapt/build comparison.

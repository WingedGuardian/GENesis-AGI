...
|---|---|
| Runtime code | `scripts/deploy_code_only.sh` (pull + reinstall + restart under the deploy-station lock, with watchdog/guardian standdown, post-restart health verify — issue #1699); full deploys via update.sh. Never the bare pull/reinstall/restart chain. On a failed health verify it ALERTS AND HOLDS — no auto-revert (moving main backwards under live sessions is the bigger hazard; owner decision 2026-09-06). Validation/E2E runs against the live server go through `scripts/run_under_deploy_lock.sh` (shared hold, so a deploy can't land mid-measurement). Cadence rule: validators SAMPLE, they don't chase — deploy `main@HEAD` at a cadence, note the SHA you validated, bisect the merge range on failure; per-PR granularity is CI's job. |

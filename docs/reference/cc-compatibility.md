...
>
> Created: 2026-03-09 | Last updated: 2026-07-04
...
| 2.1.201 | 2026-07-04 | Sonnet 5 sessions stop using the mid-conversation system role for harness reminders — behavioral change only | **Pin bumped 2.1.198→2.1.201 (this PR).** Node floor unchanged (`>=22`), no NODE_MAJOR lockstep bump. Deploy = the first single-command `scripts/update.sh` pipeline run (container `cc_ensure_local` + host gateway `update-cc` in one pass); deploy verification recorded here after the run. Watch reflection-session quality on Sonnet for a few days (harness-reminder role change). |

...
- **Foreground liveness (D3)** (`cc/foreground_reaper.py`): foreground rows stay
  ...
  **Evidence fast path + its own job:** a terminal-registered row (pid recorded
  ...
  configured interval; above an hour it switches to a CronTrigger, because an
  IntervalTrigger resets on restart. It has no boot kick of its own and needs

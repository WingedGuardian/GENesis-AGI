...
- **Merged ≠ deployed**: guardian code reaches the host ONLY via
  ...
  skill). Deploy entry points resolve one configured branch (environment,
  `~/.genesis/config/deploy.conf`, `origin/HEAD`, then `main`), refuse linked
  worktrees, detached HEAD, and branch mismatches before mutation, and merge the
  immutable fetched SHA before verifying it is active. Tracked local edits to
  deploy-managed ephemeral files are externally backed up before clearing.

...
_CALL_SITE_META: dict[str, dict] = {
    ...
    "8_ego_compaction": {
        "description": 'REMOVED FROM YAML 2026-07-13. Legacy ego rolling-summary compaction site. #26 ("Phase A — ephemeral sessions") made the ego ephemeral: CompactionEngine (ego/compaction.py) no longer performs any LLM or routing — it only persists cycles and assembles context from DB state, so it never recorded a call_site_last_run row (dead tile). Meta retained for historical reference. Note: 8_user_ego_compaction / 8_genesis_ego_compaction remain observability-only labels (NOT routing IDs) — see runtime/init/ego.py.',
        "category": "processing",
        "frequency": "Was: daily",
        "model_tier": "slm",
        "wired": False,
    ...
    "7_ego_cycle": {
        "description": 'REMOVED FROM YAML 2026-07-13. Legacy single ego cycle. #26 ("Phase A — ephemeral sessions") split the ego into two, recording under 7_user_ego_cycle (CEO/Opus) + 7_genesis_ego_cycle (COO/Sonnet). This orphaned entry only survived as ego/session.py _DEFAULT_CALL_SITE (now repointed to 7_genesis_ego_cycle). Meta retained for historical reference.',
        "category": "reasoning",
        "frequency": "Was: per ego cycle",
        "model_tier": "cc",
        "wired": False,
        "status_reason": "DEPRECATED_REMOVED",
        "see_also": ["7_user_ego_cycle", "7_genesis_ego_cycle", "40_ego_focus_selection"],
    },
    "7_user_ego_cycle": {
        "description": "User ego cycle (CEO / Opus) — periodic autonomous decision cycle for the user-facing ego. CC-dispatched via CCInvocation; records call_site_last_run under this id (provider=cc). Split from the legacy 7_ego_cycle by #26 (ephemeral sessions).",
        "category": "reasoning",
    ...
    "7_genesis_ego_cycle": {
        "description": "Genesis ego cycle (COO / Sonnet) — periodic autonomous decision cycle for Genesis's own ego. CC-dispatched via CCInvocation; records call_site_last_run under this id (provider=cc). Split from the legacy 7_ego_cycle by #26 (ephemeral sessions).",
        "category": "reasoning",
        "frequency": "Per ego cycle",
        "cost_policy": "CC background (Sonnet)",
        "model_tier": "cc",

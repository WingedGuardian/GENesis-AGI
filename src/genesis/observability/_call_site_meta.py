...
_CALL_SITE_META: dict[str, dict] = {
    ...
    "8_ego_compaction": {
        "description": "Ego-internal rolling summary compactor. Folds old ego_cycles outputs into a single compacted_summary in ego_state so long-running ego memory stays bounded. NOT Genesis-wide memory consolidation — that is the dream cycle (call site: dream_cycle_synthesis, weekly Sunday 4am). Only caller: ego/compaction.py:54. Note: 8_user_ego_compaction and 8_genesis_ego_compaction are observability labels (NOT routing IDs) — see runtime/init/ego.py:86,157.",
        "category": "processing",
        "frequency": "Daily",
        "model_tier": "slm",
        "status_reason": "WIRED",

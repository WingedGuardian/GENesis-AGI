...
_DOMAIN_REGISTRY: dict[str, SettingsDomain] = {
    ...
    "worktree_ownership": SettingsDomain(
        ...
        description=(
            ...
            "which is most of src/). LIVE AT THIS COMMIT: the Edit/Write hooks are "
            "registered, so a claim is taken on first write into an unclaimed "
            "worktree of THIS repository, a note is delivered when a session writes "
            "into one another live session holds, and claims are released at session "
            "end. NOT yet shipped: the reaper-side backstop that releases a claim "
            "whose process is gone — so a session KILLED before it can run its "
            "release leaves a claim that must be cleared by hand until that lands. "
            "Dirtiness is deliberately "

...
# Regenerate install-specific leak-detection patterns from the config just
# written. Best-effort: bootstrap.sh does this authoritatively with the venv;
# here we only run if the package is importable, else defer to bootstrap.
_VENV_PY="$(dirname "$(readlink -f "$0")")/../.venv/bin/python"
if [[ -x "$_VENV_PY" ]] && "$_VENV_PY" -c "import genesis.contribution.fingerprints" 2>/dev/null; then
    if "$_VENV_PY" -m genesis.contribution.fingerprints --write >/dev/null 2>&1; then
        _ok "Generated release fingerprints"
    else
        _warn "Could not generate release fingerprints (non-fatal — generic patterns still protect)"
    fi
else
    _print "  Release fingerprints will be generated by bootstrap.sh"
fi

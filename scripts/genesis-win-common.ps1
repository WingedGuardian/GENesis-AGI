...
function Get-GenesisHiddenTaskShimPath {
    <#
      .SYNOPSIS
        Where Install-GenesisHiddenTask writes a helper's console-suppressing
        shim. One home for the convention so install and uninstall cannot drift.
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$ScriptPath)
    [System.IO.Path]::ChangeExtension($ScriptPath, ".hidden.vbs")
}

function Remove-GenesisHiddenTaskShim {
    <#
      .SYNOPSIS
        Delete a helper's generated shim. Uninstalling the task does not do
        this - MEASURED 2026-09-07: the .vbs outlived every uninstall,
        including clean ones, so "uninstall leaves nothing behind" was true of
        the task and false of the file it was launched through.
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$ScriptPath)
    $p = Get-GenesisHiddenTaskShimPath -ScriptPath $ScriptPath
    if (Test-Path $p) { Remove-Item $p -Force -ErrorAction SilentlyContinue; return $true }
    $false
}
...
function Install-GenesisHiddenTask {
    ...
    $shimPath = Get-GenesisHiddenTaskShimPath -ScriptPath $ScriptPath

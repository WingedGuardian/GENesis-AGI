...
clean_sys_yellow() {
    ...
    find /tmp -type f -not -path "*/tmux-*" -not -name "*.sock" \
        -atime +7 -delete 2>/dev/null || true
    find /tmp -mindepth 1 -type d -empty -not -path "*/tmux-*" \
...
clean_sys_red() {
    ...
    # If still critical, remove all regular files except last 1h, sockets, tmux
    local pct_after

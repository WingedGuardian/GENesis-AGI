...
# Refuse an undeployable checkout before clearing breadcrumbs, taking the update
# lock, backing up data, or touching services.
...
if [[ "$POST_MERGE" == "true" ]]; then
    # In post-merge mode, recover the original target and rollback tag from
    # durable state. update_state is preferred; update_conflicts survives the
    # dashboard's stopped-update state cleanup during long resolutions.
...
if [[ "$POST_MERGE" == "false" ]]; then
    ...
    DEPLOY_HEAD=$(git -C "$GENESIS_ROOT" rev-parse FETCH_HEAD)
else
    ...
    if [ -z "$_saved_deploy_head" ] \
        || ! git -C "$GENESIS_ROOT" rev-parse --verify "$_saved_deploy_head^{commit}" >/dev/null 2>&1; then
        echo "ERROR: post-merge update requires the fetched deploy head saved by the initial run." >&2
        exit 1
    fi

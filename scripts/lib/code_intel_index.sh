...
# pressure. The scope cap above bounds what this job may use; it does nothing
# about WHO the kernel picks if the container as a whole runs out. This is that
# second question, and the answer is: this one. It is a batch index that can
# simply run again, and the thing that must never be killed instead of it is a CC
# session holding a user's in-flight work.
#
# Deliberately independent of the cap VALUE. This was originally written
# alongside a change raising the cap, on the reasoning that a bigger cap makes a
# bigger consumer — true, but it made the ordering look like a consequence of
# that change rather than a property worth having at any cap. Split out so each
# can be judged on its own: the kill order is correct even at the current cap,
# and the cap PR does not need this to be safe.
#

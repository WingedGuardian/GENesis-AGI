...
def main() -> None:
    ...
    print(
        ...
        "staged diff.\n"
        "EXCEPT for a docs/config-only staged set, where NO review and NO marker are "
        "required: the commit gate classifies the staged paths and exits without one. "
        "This reminder cannot tell — it hashes the staged diff without classifying it — "
        "so if everything you staged is documentation or allowlisted config, disregard "
        "the rest of this block and commit.\n\n"
        ...
        "INCLUDES a note-only or observations-only round; only new BLOCKER/SHOULD-FIX/"
        "P1/P2 advance the streak, so omitting it walks a clean review straight into "
        "the round caps below. RUN BOTH FROM THE WORKTREE THE CHANGES ARE IN — the marker is keyed "

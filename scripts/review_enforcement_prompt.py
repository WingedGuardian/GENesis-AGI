...
def main() -> None:
    ...
    print(
        ...
        "`--clean` whenever the round found no NEW should-fix-or-worse finding, which "
        "INCLUDES a note-only or observations-only round. Should-fix-or-worse is PER "
        "REVIEWER VOCABULARY: genesis-architect -> BLOCKER/SHOULD-FIX; "
        "genesis-security-reviewer -> CRITICAL/WARNING (WARNING is its should-address "
        "tier, NOT a note); Codex -> P1/P2. That decides whether you pass `--clean`; the "
        "tool does NOT read severities, it trusts your flag. WRITING the evidence is a "
        ...
        "text or the depth gate rejects the evidence as non-adversarial. Omitting "
        "`--clean` walks a clean review into the round caps below; adding it after a "
        "round that raised CRITICAL or WARNING silently RESETS the escalation counter "
        "those caps exist to enforce. "

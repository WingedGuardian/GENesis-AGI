...
for pattern in "${patterns[@]}"; do
    if rg -n --hidden --glob '!**/.git/**' --glob '!**/.claude/**' --glob '!**/logs/**' --glob '!**/docs/**' --glob '!**/CLAUDE.md' --glob '!**/genesis-container-setup.md' --glob '!**/scripts/check_portability.sh' --glob '!**/scripts/cc_cli_output/**' --glob '!**/scripts/spike_*' "$pattern" "${targets[@]}" >"$HOME/tmp/genesis-portability-scan.txt"; then

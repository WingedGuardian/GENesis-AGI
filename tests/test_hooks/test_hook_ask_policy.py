...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("HEAD", "feat/x", True, "THE missing spelling - resolves to the checked-out branch"),
        ...
        ("refs/heads/main", "feat/x", False, "same, fully qualified - the P1-A spelling"),
...
def test_which_refspecs_name_the_current_branch(ref, cur, names_it: bool, why: str) -> None:
    """The root fix, driven directly.

    ``HEAD`` was the missing spelling and its absence was not cosmetic. MEASURED
    on a pristine origin/main copy: ``git push -u origin HEAD`` returned False
    from ``_push_targets_current_branch``, which sends it down main()'s catch-all
    arm - so ``_push_is_republish``, the no-open-PR hygiene check and the
    close-then-push check never ran for the command this repo's own development
    skill prescribes, and the one both of today's PRs were published with.

    The False rows are what keeps this a narrowing rather than a loosening: a
    tag, another branch, and ``refs/heads/main`` from a feature branch all stay
    unrecognised and keep prompting.
    """
...
def test_the_refspec_rule_reaches_push_targets_current_branch(
    command: str, targets_cur: bool
) -> None:
    """The binding test: ``_ref_names_current_branch`` is only useful if
    ``_push_targets_current_branch`` actually consults it. Driven through the
    real parse, so a change that defines the helper and never calls it fails
    here rather than passing as a unit."""
...
def test_the_catch_all_arm_is_wholly_unsuppressible(
    ...
    """Whatever lands in the catch-all keeps prompting, with no predicate deciding.

    This replaces a predicate that tried to pick the routine publishes back out
    of that arm and was wrong twice in two review rounds - ``refs/heads/main``
    and tags both walked through its literal main/master check. The arm is
    defined by NEGATION, so classifying it means enumerating a space with no
    edge; the fix is that routine publishes no longer land here at all."""
    repo = tmp_path / "repo"
    repo.mkdir()
    for args in (
        ["init", "--quiet", "-b", "feat/x"],
    ...
    monkeypatch.setattr(
        ...
        "stdin",
        io.StringIO(
            json.dumps({"tool_name": "Bash", "tool_input": {"command": command}, "cwd": str(repo)})
        ),
    )
    rc = gpg.main()
    out = capsys.readouterr()
    assert rc in (0, 2), (rc, out.out, out.err)
    if out.out.strip():
        assert _decision(json.loads(out.out)) != "allow", out.out

...
class TestSeverityParsing:
    ...
    def test_every_documented_level_is_read(self, guard_module, token, expected):
        # VERIFY-RED anchor: with a `\b` boundary after the severity word this
        # returns None for EVERY case, because the header's own delimiter is `_`
        # and underscore IS a regex word character. Measured 0/27 classified on
        # live data before the lookarounds replaced `\b`.
    ...
    def test_severity_word_inside_a_longer_word_does_not_match(self, guard_module):
        # `Majority` must not read as `major` — the lookarounds are ASCII-letter
        # bounded precisely so a substring cannot borrow a severity.
...
class TestSectionParsing:
    ...
    def test_a_following_section_ends_the_outside_one(self, guard_module):
        ...
        entries, _declared = guard_module._cr_outside_diff_entries(body)
...
class TestGateBehaviour:
    ...
    def test_critical_outside_the_diff_is_not_scored(
        ...
        # #1728: a base-branch finding is not this PR's to answer.
...
class TestQuotedContent:
    ...
    def test_a_fenced_summary_does_not_reassign_the_current_file(
        ...
        entries, _ = guard_module._cr_outside_diff_entries(body)
...
class TestStructuralAttribution:
    ...
    def test_unfenced_summary_in_prose_does_not_reassign_the_file(self, guard_module):
        ...
        entries, declared = guard_module._cr_outside_diff_entries(body)

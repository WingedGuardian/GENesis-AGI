...
def test_load_full_yaml(monkeypatch):
    ...
    # 2026-09-05: 26 -> 27 with mistral-medium-free — the in-family rung
    # directly below Large (presumptive free tier; see model_routing.yaml).
    # Both of the above landed independently and this merge carries BOTH, so the
    # count moved twice. The two branches each wrote "25 -> 26"; only the comment
    # conflicted, while the assertion below auto-merged at 26 and was silently
    # wrong for the merged config until this resolution.

"""Migration 0090 — void historical owner-facing outreach predictions.
...
M90 = importlib.import_module("genesis.db.migrations.0090_void_owner_facing_outreach_predictions")
...
async def _build(conn: aiosqlite.Connection) -> None:
    ...
    await conn.execute(
        ...
        "  outcome_value INTEGER, resolver TEXT, brier REAL)"
    )
    for oid, ch in [
        ("oh-tg", "telegram"),
        ("oh-voice", "voice"),
    ...
    for r in rows:
        await conn.execute("INSERT INTO ledger_predictions VALUES (?, ?, ?, ?, ?, ?, ?, ?)", r)
    await conn.commit()
...
async def _status(db, pid):
    cur = await db.execute(
        "SELECT status, resolved_at, outcome_value, resolver, brier "
...
async def test_voids_owner_facing_keeps_external(db):
    ...
    # A voided-from-resolved row CLEARS its grade, so it satisfies the ledger invariant
    # "outcome_value non-None iff status='resolved'" (no void row carrying a stale grade).
    ...
    # External channels (email/discord) → fully untouched, grade preserved.

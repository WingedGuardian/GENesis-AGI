...
Four levels of verification, ordered by increasing rigor. Assess every component
under review against the highest level it claims to satisfy.

### Level 1: Exists
Code is written and present. Files exist, classes are defined, functions have bodies.
Minimum bar — proves nothing about correctness or integration.

**Review question:** Does the file/function/class exist?

### Level 2: Substantive
Code does something meaningful when called. Unit tests pass. Logic handles happy
path and at least one error path. Nothing proves it runs in the actual system.

**Review question:** Do tests pass? Does the logic handle basic edge cases?

### Level 3: Wired
Code has at least one live call site in the actual runtime path — not just a test
that mocks its callers. The import chain from entry point to this code is unbroken.

**Review question:** What calls this? Show the line. Trace from bootstrap/entry
point to this code.

### Level 4: Data-Flow Verified
Data flows through this code end-to-end in a running system. An integration test
or manual verification proves real inputs produce real outputs through the actual
call chain. Init ordering is correct. Config is loaded. Dependencies injected.

**Review question:** Can you show evidence of real data flowing through this path?
If the system restarts, does this still work?

### Using the Taxonomy

When reviewing a PR claiming a component is "done":
- **Level 1 only** (exists) — flag as incomplete
- **Level 2 only** (substantive) — flag: "where is the call site?"
- **Level 3** (wired) — acceptable for most components
- **Level 4** (data-flow) — required for critical paths: bootstrap init, data
  persistence, external API integration, autonomy actions

Maps to the "built ≠ wired ≠ verified" principle in CLAUDE.md.
